...
public class MermaidUtils {
     ...
     *
     * @param typeHierarchy List of layers of Thing objects representing the
     *                      supertypes.
            ...
            while (hierarchyIterator2.hasPrevious()) {
                ...
                if (topLevelToggle) {
                    for (Thing thing : nextNodes) {
                        final String nodeName = thing.getPredicates().get(RDFS.RDF_TYPE).iterator().next().toString()
                                .split("#")[1];
                        ...
                        if (supertypes != null) {
                            for (Object st : supertypes) {
                                ...
                                String stName = stThing.getPredicates().get(RDFS.RDF_TYPE).iterator().next().toString()
                                        .split("#")[1];
                                if (stName.equals("class")) {
                                    stName = "Class";
                                }
                                String nodeNameMod;
                        ...
                        if (superclasses != null) {
                            ...
                            for (Object sc : superclasses) {
                                ...
                                if (scThing != null) {
                                    String stName = scThing.getPredicates().get(RDFS.RDF_TYPE).iterator().next()
                                            .toString().split("#")[1];
                                    if (stName.equals("class")) {
                                        stName = "Class";
                                    }
                                    ...
                                    if (stName != "" && (!stName.equals(nodeNameMod))) {
                                        mermaidNodeLine += (stName + ROUNDED_START + insertBRifTooLong(stName)
                                                + ROUNDED_END +
                                                SUPERTYPE_OF +
                                                nodeNameMod + ROUNDED_START + insertBRifTooLong(nodeNameMod)
                                                + ROUNDED_END);
                                        bWriter.write(mermaidNodeLine + ";");
                                        bWriter.newLine();
                                    }
                ...
                if (!hierarchyIterator2.hasPrevious()) {
                    for (Thing nn : nextNodes) {
                        bWriter.write(STYLE_START
                                + nn.getPredicates().get(RDFS.RDF_TYPE).iterator().next().toString().split("#")[1]
                                + STYLE_END);
                    }
                }
            }
    ...
    /**
     * Compute the left-to-right node-edge graph and write to file using Mermaid
     * notation
     *
     * @param nodeEdgeGroups List of layers of Thing objects representing the node
     *                       groups.
     */
    public static void writeLRNodeEdgeGraph(final List<Thing> nodeEdgeGroups,
        ...
        try {
            final FileWriter fWriter = new FileWriter(
                    new File("example-files/" + exampleName + "NodeEdgeGraph.mermaid"));
            final BufferedWriter bWriter = new BufferedWriter(fWriter);
                ...
                // for(Thing thing : nextNodes){
                {
                    ...
                    nodeEdgesMap.forEach((predicate, objects) -> {
                        ...
                        if (!predicatesToExclude.contains(predicateName)) {
                            ...
                            final String thingName = (thingNameShort + CIRCLE_START + "\"" + thingNamePrint + "\""
                                    + CIRCLE_END);
                            ...
                            objects.forEach(obj -> {
                                ...
                                if (obj.toString().contains("#")) {
                                    ...
                                    final List<String> nameObjArray = Arrays
                                            .asList(objNameShort.split("(?<=\\G.{18})"));
                                    ...
                                    if (objNamePrefix.contains("hqdm")) {
                                        objName = objNameShort
                                                + (STADIUM_START + "\"" + objNamePrint + "\"" + STADIUM_END);
                                    } else {
                                        objName = objNameShort
                                                + (CIRCLE_START + "\"" + objNamePrint + "\"" + CIRCLE_END);
                                    }
                                } else {
                                    String addB = "";
                                    ...
                                    String nodeName = tmpName.replace('@', 'a');
                                    objName = nodeName + "[\"" + addB + insertBRifTooLong(tmpName) + "\"]";
                                }
                                ...
                                mermaidNodeLine += (thingName + "--->|" + "\"" + predicatePrefix + predicateName + "\""
                                        + "|" + objName);
         ...
         * ``` mermaid
         * graph LR
         * id1((patterns: <br> 7a14739a-7bdd-49a0- <br> a935-3ef41f0cf955))
         * --->|patterns-rdl:record_created| p1(2023-08-01T10:40:51.880960800Z);
         * id1 --->|patterns-rdl:record_creator| p2(HqdmPatternProject_User1);
         * id1 --->|hqdm:EntityName| p3(Example_Thing_As_Instance_Of_ <br>
         * TopLevelHQDM_EntityType_THING);
         * ```
         */
    }
    ...
    private static String insertBRifTooLong(final String inputString) {
        if (inputString.length() > NAME_LENGTH_BREAK + 1) {
            ...
            if (index != -1) {
                ...
                return (inputString.substring(0, NAME_LENGTH_BREAK) + " <br> "
                        + inputString.substring(NAME_LENGTH_BREAK));
            } else {
                return inputString;
            }
