...
public class AssociationExample {
    ...
    public static void createAndAddAssociationPattern(final List<MagmaCoreService> mcDatasets) {
        ...
        // Find supertypes
        List<List<Thing>> supertypes = FindSupertypes.findSuperTypes(List.of("association", "participant", "kind_of_association", "role"));
        ...
        final Thing kindOfGenericAssociationObject = PatternsUtils.createNewBaseObject(new HqdmObjectBaseProperties(
        ...
        kindOfGenericAssociationObject.addStringValue(PatternsUtils.COMMENT, "Kind_of_association_for_generic_pattern_examples");
        ...
        individualYObject.addValue(HQDM.PART_OF_POSSIBLE_WORLD, QueryUtils.findThingInServiceByName(mcDatasets, "Possible_World_for_generic_pattern_examples").getId());
        individualYObject.addValue(HQDM.BEGINNING, QueryUtils.findThingInServiceByName(mcDatasets, "t0").getId());
        ...
        stateOfYParticipantObject.addValue(HQDM.PART_OF_POSSIBLE_WORLD, QueryUtils.findThingInServiceByName(mcDatasets, "Possible_World_for_generic_pattern_examples").getId());
        stateOfYParticipantObject.addValue(HQDM.BEGINNING, QueryUtils.findThingInServiceByName(mcDatasets, "t1").getId());
        stateOfYParticipantObject.addValue(HQDM.ENDING, QueryUtils.findThingInServiceByName(mcDatasets, "t2").getId());
        ...
        associationXYObject.addValue(HQDM.BEGINNING, QueryUtils.findThingInServiceByName(mcDatasets, "t1").getId());
        associationXYObject.addValue(HQDM.ENDING, QueryUtils.findThingInServiceByName(mcDatasets, "t2").getId());
        associationXYObject.addValue(HQDM.PART_OF_POSSIBLE_WORLD, QueryUtils.findThingInServiceByName(mcDatasets, "Possible_World_for_generic_pattern_examples").getId());
        ...
        MermaidUtils.writeLRNodeEdgeGraph(List.of(associationXYObject, stateOfXParticipantObject, stateOfYParticipantObject ), 
        ...
        MermaidUtils.writeLRNodeEdgeGraph(List.of(
                ...
                roleOfYThingObject ), 
                List.of("record_created", "record_creator", "comment"), 
                List.of(associationXYObject.getId().split("#")[1], stateOfXParticipantObject.getId().split("#")[1], stateOfYParticipantObject.getId().split("#")[1]),
                "genericAssociationAndParticipantsFull");

        try {
            final PrintStream ttl_stream_out = new PrintStream("example-files/associationGenericPattern.ttl");
            final PrintStream stmt_stream_out = new PrintStream("example-files/associationGenericPattern.stmt");

            associationService.exportTtl(ttl_stream_out);
            ttl_stream_out.close();
            System.out.println("\tData being generated as TTL in example-files/associationGenericPattern.ttl.");
