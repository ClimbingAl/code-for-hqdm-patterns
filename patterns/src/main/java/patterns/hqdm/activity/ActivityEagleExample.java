...
public class ActivityEagleExample {
    ...
    public static void loadEagleActivityFromEditor(final List<MagmaCoreService> mcDatasets) {
        ...
        try {
            InputStream activityInputStream = new FileInputStream("input-files/activity_diagram_state_of_apollo11_mission.ttl");
        ...
        final Thing participantBuzz = 
            eagleActivityService.getInTransaction( new IRI(PatternsUtils.ACTIVITY_EDITOR_BASE, "d1f7335c-c255-4358-9dc3-2d78aa3ba81d")); // Buzz Aldrin participant state
        final Thing participantLM5AscentModule = 
            eagleActivityService.getInTransaction( new IRI(PatternsUtils.ACTIVITY_EDITOR_BASE, "ab9fd44e-ff12-4a21-915a-849816c5198e")); // LM-5 Ascent Module participant state
        final Thing participantNeil = 
            eagleActivityService.getInTransaction( new IRI(PatternsUtils.ACTIVITY_EDITOR_BASE, "6b50a479-07ca-4cac-a65d-f3813259b8d9")); // Neil Armstrong participant state

        final Thing lm5AscentModule = 
            eagleActivityService.getInTransaction( new IRI(PatternsUtils.ACTIVITY_EDITOR_BASE, "4974518d-53c0-4f56-8017-3a989bf36065"));
        final Thing neilArmstrongPerson = 
            eagleActivityService.getInTransaction( new IRI(PatternsUtils.ACTIVITY_EDITOR_BASE, "a589d9e5-919f-476d-aa54-8c6dac941061"));
        final Thing buzzAldrinPerson = 
            eagleActivityService.getInTransaction( new IRI(PatternsUtils.ACTIVITY_EDITOR_BASE, "7bbf504e-f834-40dc-b060-974827fbef76"));        
        ...
        MermaidUtils.writeLRNodeEdgeGraph(List.of(
            ...
            List.of(returnFromLunarSurfaceActivity.getId().split("#")[1], 
                participantBuzz.getId().split("#")[1], 
                participantNeil.getId().split("#")[1],
                participantLM5AscentModule.getId().split("#")[1] ),
            "activityLM5AscentAndParticipants");

        final Thing kindOfLunarAscentObject = 
            eagleActivityService.getInTransaction( new IRI(PatternsUtils.ACTIVITY_EDITOR_BASE, "b41158ca-2619-4097-9cf5-890979343127"));
        final Thing roleOfLunarLanderLMPObject = 
            eagleActivityService.getInTransaction( new IRI(PatternsUtils.ACTIVITY_EDITOR_BASE, "becf0e5b-f094-43eb-be03-a36de244a05d"));
        final Thing roleOfLunarLanderCMDObject = 
            eagleActivityService.getInTransaction( new IRI(PatternsUtils.ACTIVITY_EDITOR_BASE, "9f55ec51-3926-46bc-9c1e-a96f1379ce14"));
        final Thing roleOfLunarLanderObject = 
            eagleActivityService.getInTransaction( new IRI(PatternsUtils.ACTIVITY_EDITOR_BASE, "b4eebfca-cd08-40b4-a6f9-bfe5f793c93c"));
        final Thing beginning1 = 
                    eagleActivityService.getInTransaction( new IRI(PatternsUtils.ACTIVITY_EDITOR_BASE, "a0f6879b-1127-438d-bb78-43ae5de9d2c5"));
        final Thing ending1 = 
                    eagleActivityService.getInTransaction( new IRI(PatternsUtils.ACTIVITY_EDITOR_BASE, "158e354d-ffbb-481c-981f-b8d609860a69"));
